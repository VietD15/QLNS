    ...
    <div class="row">
        ...
        <div class="row">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Số Điểm Thưởng</th>
                                </tr>
                            </thead>
                            <tbody>
                                <template v-for="(value, index) in list_diem_thuong" :key="index">
                                    <tr>
                                        <th class="text-center">{{ index + 1 }}</th>
                                        <td>{{ value.ho_va_ten }}</td>
                                        <td class="text-center">{{ value.so_diem_thuong }}</td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mt-2">Thống Kê Số Điểm Thưởng Của Từng Nhân Viên</h5>
                    </div>
                    <div class="card-body">
...
export default {
    ...
    methods: {
        thongKe() {
            this.is_load = false
            axios
                .post('/api/admin/diem-thuong/thong-ke', this.day, {
                    headers: {
                        Authorization: 'Bearer ' + localStorage.getItem("tk_nhan_vien")
                    }
                })
                .then((res) => {
                    this.list_diem_thuong = res.data.data;
                    this.chartData.labels = res.data.ten_nhan_vien;
                    this.chartData.datasets[0].data = res.data.tong_so_diem_thuong;
                    this.is_load = true;
